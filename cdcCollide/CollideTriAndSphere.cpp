...
int32_t CollideTriAndSphere(
	...
	Vector3 weakAvg = {(tri.v0 + tri.v1 + tri.v2) * (1.0000119f / 3)};
	...
	if (inside10 <= 0.f) {
		if (dot_h10_h1_minus_p >= 0.f && dot_h10_p_minus_h0 >= 0.f) {
			if (adjacencyFlags & 8) {
				Vector3 cp;
				float lambda;
				float distSqr = PointToSegmentDistance(&lambda, &cp, p, h0, h1);
				if (radius * radius >= distSqr) {
					Vector3 edgeNormal = cp - p;
					if (edgeNormal.LenSquared() < 1e-6f)
						edgeNormal = triNormal;
					edgeNormal.Normalize3();
					contacts->position = cp - edgeNormal * radius;
					contacts->normal = edgeNormal;
	...
	if (inside21 <= 0.f) {
		if (dot_h21_h2_minus_p >= 0.f && dot_h21_p_minus_h1 >= 0.f) {
			if (adjacencyFlags & 0x10) {
				Vector3 cp;
				float lambda;
				float distSqr = PointToSegmentDistance(&lambda, &cp, p, h1, h2);
				if (radius * radius >= distSqr) {
					Vector3 edgeNormal = cp - p;
					if (edgeNormal.LenSquared() < 1e-6f)
						edgeNormal = triNormal;
	...
	if (inside02 <= 0.f) {
		if (dot_h02_h0_minus_p >= 0.f && dot_h02_p_minus_h2 >= 0.f) {
			if (adjacencyFlags & 0x20) {
				Vector3 cp;
				float lambda;
				float distSqr = PointToSegmentDistance(&lambda, &cp, p, h2, h0);
				if (radius * radius >= distSqr) {
					Vector3 edgeNormal = cp - p;
					if (edgeNormal.LenSquared() < 1e-6f)
						edgeNormal = triNormal;
