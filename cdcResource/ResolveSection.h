...
public:
	...
	virtual void* getWrapped(uint32_t) = 0;     // 0x30 (maybe GetBasePointer)
	virtual void* getBlob(uint32_t);            // 0x34 (maybe GetResolveBasePointer)
	virtual uint32_t getDomainId(uint32_t) = 0; // 0x44 (maybe FindResource)
};
